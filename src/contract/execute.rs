...
use crate::contract::{
    error::ContractError,
    msg::{ExecuteMsg, CustomExecuteMsg},
        ...
        ExecuteMsg::Custom(msg) => {
            match msg {
                CustomExecuteMsg::SetPixelColor { 
                    ...
                    current_metadata, 
                    updates 
                } => set_pixel_color::set_pixel_color(deps, env, info, token_id, current_metadata, updates),
                CustomExecuteMsg::UpdateConfig { 
                    tile_royalty_payment_address, 
                    tile_royalty_fee_percent, 
                    price_scaling 
                } => update_config::update_config(deps, env, info, tile_royalty_payment_address, tile_royalty_fee_percent, price_scaling),
            }
        },
        ExecuteMsg::Base(base_msg) => {
            match base_msg {
                sg721::ExecuteMsg::Mint { token_id, owner, token_uri, .. } => {
                    tiles::mint::mint(deps, env, info, token_id, owner, token_uri)
                },
                _ => contract.execute(deps, env, info, base_msg)
