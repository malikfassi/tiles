...
use tiles::{
    core::{
        ...
        tile::metadata::PixelUpdate,
    },
    events::{
        EventData, InstantiateConfigEventData, MintMetadataEventData,
        PixelUpdateEventData, PaymentDistributionEventData,
        PriceScalingUpdateEventData,
    },
};
...
impl EventAssertions {
    ...
    pub fn extract_token_id(response: &AppResponse) -> u32 {
        let events = Self::find_events(response, MintMetadataEventData::event_type().as_wasm_str().as_str());
    ...
    pub fn assert_mint_metadata(
        ...
        assert_eq!(parsed.token_id.parse::<u32>().unwrap(), token_id, "Token ID mismatch");
    ...
    pub fn assert_instantiate_config(
        response: &AppResponse,
        expected_minter: &str,
    ) {
        let event = Self::find_event(
            response,
            InstantiateConfigEventData::event_type().as_wasm_str().as_str(),
        );
        ...
        assert!(!parsed_event.collection_info.is_empty(), "Collection info should not be empty");
        assert_eq!(parsed_event.minter, expected_minter, "Minter address mismatch");
        assert!(!parsed_event.price_scaling.is_empty(), "Price scaling should not be empty");
    ...
    pub fn assert_pixel_update(
        ...
        assert_eq!(events.len(), updates.len(), 
            "Expected exactly {} pixel update events, found {}", updates.len(), events.len());
        ...
        for update in updates {
            ...
            assert_eq!(parsed_event.token_id, token_id.to_string(), "Token ID mismatch");
            assert_eq!(parsed_event.pixel_id, update.id, "Pixel ID mismatch");
            assert_eq!(parsed_event.color, update.color, "Color mismatch");
            assert_eq!(parsed_event.expiration_duration, update.expiration_duration, "Duration mismatch");
            assert_eq!(parsed_event.last_updated_by, sender.to_string(), "Sender mismatch");
    ...
    pub fn assert_payment_distribution(
        ...
        let event = Self::find_event(
            response,
            PaymentDistributionEventData::event_type().as_wasm_str().as_str(),
        );
        let parsed_event = PaymentDistributionEventData::try_from_event(event).unwrap_or_else(|| {
            panic!(
        ...
        assert_eq!(parsed_event.royalty_amount.to_string(), royalty_amount.to_string(), "Royalty amount mismatch");
        assert_eq!(parsed_event.owner_amount.to_string(), owner_amount.to_string(), "Owner amount mismatch");
    }

    pub fn assert_price_scaling_update(response: &AppResponse, scaling: &PriceScaling) {
        let event = Self::find_event(
            response,
            PriceScalingUpdateEventData::event_type().as_wasm_str().as_str(),
        );
        let parsed_event = PriceScalingUpdateEventData::try_from_event(event).unwrap_or_else(|| {
            panic!(
        ...
        assert_eq!(parsed_event.hour_1_price.to_string(), scaling.hour_1_price.u128().to_string(), "Hour 1 price mismatch");
        assert_eq!(parsed_event.hour_12_price.to_string(), scaling.hour_12_price.u128().to_string(), "Hour 12 price mismatch");
        assert_eq!(parsed_event.hour_24_price.to_string(), scaling.hour_24_price.u128().to_string(), "Hour 24 price mismatch");
        assert_eq!(parsed_event.quadratic_base.to_string(), scaling.quadratic_base.u128().to_string(), "Quadratic base mismatch");
    }
} 