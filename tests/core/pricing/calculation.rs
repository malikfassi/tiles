...
fn price_at_6_hours_is_interpolated() {
    ...
    assert_eq!(price, expected, "Price at 6 hours should match linear interpolation");
}
...
fn price_at_18_hours_is_interpolated() {
    ...
    assert_eq!(price, expected, "Price at 18 hours should match linear interpolation");
}
...
fn total_price_sums_individual_prices() {
    let scaling = PriceScaling::default();
    let durations = vec![ONE_HOUR / 2, ONE_HOUR * 6];
